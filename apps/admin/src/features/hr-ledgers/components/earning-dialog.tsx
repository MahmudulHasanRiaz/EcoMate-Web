...
import { useCreateEarningMutation } from '../hooks'
import type { EarningType } from '../api'
...
export function EarningDialog({ employeeId }: { employeeId: string }) {
  const [open, setOpen] = useState(false)
  const [type, setType] = useState<EarningType>('bonus')
  const [amount, setAmount] = useState('')
  const [reason, setReason] = useState('')
  const [from, setFrom] = useState<Date | undefined>()
  const [to, setTo] = useState<Date | undefined>()

  const createMut = useCreateEarningMutation(employeeId)
  ...
  return (
    <Dialog open={open} onOpenChange={(o) => { if (!o) reset(); setOpen(o) }}>
      <DialogTrigger asChild>
        <Button size='sm'>
          <span className='mr-1'>+</span> Add Earning
        </Button>
      </DialogTrigger>
      <DialogContent className='sm:max-w-[520px]'>
        ...
        <div className='grid gap-4 py-4'>
          <div className='grid gap-2'>
            <Label>Type</Label>
            <Select value={type} onValueChange={(v) => setType(v as EarningType)}>
          ...
          <div className='grid gap-2'>
            <Label>Reason</Label>
            <Input value={reason} onChange={(e) => setReason(e.target.value)} placeholder='Reason for earning' />
          </div>
