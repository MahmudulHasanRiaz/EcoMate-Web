...
import { Pencil, Plus, Radio, Trash2, Undo2 } from 'lucide-react'
...
export function MetaDestinationsCard({ raw, onChange, disabled }: Props) {
  ...
  // Inline expansion: rows stay compact; details open for Add and Edit only.
  ...
  const addDestination = () => {
    ...
    const id = newDestinationId(drafts)
    const next = [
      ...drafts,
      {
        id,
    ...
    onChange(JSON.stringify(next))
    setErrors(validateDrafts(next))
    // A new row has empty required fields — open it immediately for editing.
    setExpandedId(id)
  }
  ...
  const removeDestination = (index: number) => {
    ...
    if (id && expandedId === id) setExpandedId(null)
  ...
  return (
    <Card className='overflow-hidden border-none shadow-md bg-gradient-to-br from-background to-muted/20'>
      <CardHeader className='pb-4'>
        ...
        <CardDescription>
          One event is delivered to every enabled destination with the same event ID.
          Tokens stay server-side and never reach the storefront.
        </CardDescription>
      </CardHeader>
      <CardContent className='space-y-4'>
        ...
        {drafts.map((d, index) => {
          const expanded = expandedId === d.id && drafts.some((x) => x.id === d.id)
          return (
            <div
              key={d.id}
              className={`rounded-lg border ${d.removedAt ? 'opacity-60 border-dashed' : ''} ${expanded ? 'p-4 space-y-3' : 'px-3 py-2.5'}`}
            >
              {/* Compact row: label + pixel id + status, Edit/Remove actions. */}
              <div className='flex items-center gap-2'>
                <div className='min-w-0 flex-1'>
                  <div className='flex min-w-0 items-center gap-2'>
                    <span className='truncate text-sm font-medium'>{d.label || d.id}</span>
                    {d.removedAt ? (
                    ...
                    {!d.removedAt && !d.browserPixelEnabled && (
                      <Badge variant='outline'>No browser pixel</Badge>
                    )}
                    {!d.removedAt && d.testMode && (
                      <Badge variant='outline'>Test</Badge>
                    )}
                  </div>
                  <div className='mt-0.5 truncate font-mono text-xs text-muted-foreground'>
                    {d.pixelId || <span className='font-sans italic'>no pixel id yet</span>}
                    <span className='ml-2 font-sans'>· {d.id}</span>
                  </div>
                </div>
                {d.removedAt ? (
                  <Button type='button' variant='ghost' size='sm' onClick={() => restoreDestination(index)} disabled={disabled} aria-label={`Restore destination ${d.label || d.id}`}>
                  ...
                  <>
                    <Button
                      ...
                      aria-label={`${expanded ? 'Close' : 'Edit'} destination ${d.label || d.id}`}
                    >
                      <Pencil className='h-4 w-4 mr-1' /> {expanded ? 'Close' : 'Edit'}
                    </Button>
                    <Button type='button' variant='ghost' size='sm' onClick={() => removeDestination(index)} disabled={disabled} aria-label={`Remove destination ${d.label || d.id}`}>
