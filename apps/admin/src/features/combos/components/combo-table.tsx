...
import { combosColumns } from './combo-columns'

type Props = {
  data: ComboResponse[]; pageCount: number; pagination: PaginationState;
  onPaginationChange: (p: PaginationState) => void; isLoading?: boolean;
  onEdit: (row: ComboResponse) => void; onDelete: (row: ComboResponse) => void;
}

export function CombosTable({ data, pageCount, pagination, onPaginationChange, isLoading, onEdit, onDelete }: Props) {
  const [sorting, setSorting] = useState<SortingState>([])
  const [columnVisibility] = useState<VisibilityState>({})

  const columns = combosColumns(onEdit, onDelete)
        ...
        <Table>
          ...
          <TableBody>
            {isLoading ? (
              ...
              <TableRow><TableCell colSpan={10} className='h-24 text-center text-muted-foreground'>No combos found.</TableCell></TableRow>
            )}
          </TableBody>
        </Table>
      </div>
