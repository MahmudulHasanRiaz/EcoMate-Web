...
describe('UsersService', () => {
  ...
  describe('create', () => {
    ...
    it('should create a user successfully', async () => {
      ...
      ;(prisma.userSettings.create as jest.Mock).mockResolvedValue({ id: 'settings-2', userId: 'user-id-2' })

      const result = await service.create(createDto)

      expect(prisma.user.findFirst).toHaveBeenCalledWith({
        where: { OR: [{ email: createDto.email }, { username: createDto.username }] },
      })
      expect(bcrypt.hash).toHaveBeenCalledWith(createDto.password, 12)
      expect(prisma.user.create).toHaveBeenCalled()
      expect(prisma.userSettings.create).toHaveBeenCalledWith({ data: { userId: 'user-id-2' } })
      expect(result).not.toHaveProperty('password')
    })
  ...
  describe('update', () => {
    ...
    it('should throw ConflictException if new email is already taken', async () => {
      ;(prisma.user.findUnique as jest.Mock)
        .mockResolvedValueOnce(mockUserResponse)
        .mockResolvedValueOnce({ id: 'other-user', email: 'johnny@example.com' })

      await expect(service.update('user-id-1', { email: 'johnny@example.com' })).rejects.toThrow(
        ConflictException,
      )
    })
    ...
    it('should throw ConflictException if new username is already taken', async () => {
      ...
      await expect(service.update('user-id-1', { username: 'takenuser' })).rejects.toThrow(
        ConflictException,
      )
    })
