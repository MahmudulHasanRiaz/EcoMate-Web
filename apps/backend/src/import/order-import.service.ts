    ...
    await this.prisma.$transaction(async (tx) => {
      const order = await tx.order.create({
        data: {
          ...
          // Order-time customer snapshot for imported orders
          customerEmail: row.billingEmail || null,
          customerPhone: row.billingPhone || null,
          customerFirstName: row.shippingFirstName || customerName?.split(' ')[0] || null,
          customerLastName: row.shippingLastName || customerName?.split(' ').slice(1).join(' ') || null,
          customerCity: row.shippingCity || null,
          customerState: row.shippingState || null,
          customerZip: row.shippingPostcode || null,
          customerCountry: row.shippingCountry || 'BD',
