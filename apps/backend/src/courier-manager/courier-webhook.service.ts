...
const ADVANCED_STATUSES = new Set([
  'In Courier', 'Shipped', 'Delivered', 'Return Pending', 'Partial Return', 'Returned', 'Damaged',
]);
...
export class CourierWebhookService {
  ...
  async handleSteadfast(body: Record<string, unknown>) {
    ...
    if (this.isRegression(order.courierStatus || '', mappedStatus)) {
      this.logger.warn(`Steadfast regression blocked: ${consignmentId} ${order.courierStatus} → ${mappedStatus}`);
    ...
    await this.prisma.order.update({
      ...
      data: {
        ...
        courierTrackingCode: (body['tracking_code'] as string) || order.courierTrackingCode,
  ...
  async handleRedx(body: Record<string, unknown>) {
    ...
    let order: { id: string; courierStatus: string | null; courierTrackingCode: string | null; courierConsignmentId: string | null } | null = null;
    if (trackingId) order = await this.prisma.order.findFirst({ where: { courierTrackingCode: trackingId } });
    if (!order && invoiceId) order = await this.prisma.order.findFirst({ where: { displayId: invoiceId } });
    if (!order) return { error: 'Order not found' };
    ...
    if (this.isRegression(order.courierStatus || '', mappedStatus)) {
      this.logger.warn(`RedX regression blocked: ${trackingId} ${order.courierStatus} → ${mappedStatus}`);
      return { ok: true, skipped: 'Status regression blocked' };
    }
  ...
  async handleCarrybee(body: Record<string, unknown>) {
    ...
    let order: { id: string; courierStatus: string | null; courierTrackingCode: string | null; courierConsignmentId: string | null } | null = null;
    if (consignmentId) order = await this.prisma.order.findFirst({ where: { courierConsignmentId: consignmentId } });
    if (!order && orderNumber) order = await this.prisma.order.findFirst({ where: { displayId: orderNumber } });
    if (!order) return { error: 'Order not found' };
  ...
  private isRegression(currentStatus: string, newStatus: string): boolean {
    ...
    if (currentStatus === 'Delivered' && (newStatus === 'Returned' || newStatus === 'Return Pending')) return false;
    return false;
  }
...
function mapPathaoStatus(raw?: string | null): string | null {
  ...
  if (['order.picked', 'order.at-the-sorting-hub', 'order.in-transit', 'order.received-at-last-mile-hub', 'order.assigned-for-delivery', 'order.on-hold'].includes(s)) return 'In Courier';
  ...
  if (['order.returned', 'order.delivery-failed', 'order.paid-return'].includes(s)) return 'Return Pending';
  return null;
}
...
function mapRedxStatus(raw?: string | null): string | null {
  if (!raw) return null;
  const s = raw.toLowerCase().replace(/[\s-]+/g, '_');
  if (['pending', 'picked', 'in_transit', 'ofd', 'out_for_delivery', 'hold'].includes(s)) return 'In Courier';
...
function mapCarrybeeStatus(event?: string | null, deliveryStatus?: string | null): string | null {
  const raw = event || deliveryStatus;
  if (!raw) return null;
  const slug = raw.toLowerCase().replace(/[\s-]+/g, '-');
  if (slug === 'order.pickup-cancelled') return null;
  if (['order.picked', 'order.assigned-for-pickup', 'order.pickup-requested'].includes(slug)) return 'In Courier';
  ...
  if (['order.delivery-failed', 'order.returned', 'order.returned-at-sorting', 'order.returned-to-merchant'].includes(slug)) return 'Return Pending';
  const s = (deliveryStatus || '').toLowerCase().replace(/[\s-]+/g, '_');
  if (['pending', 'picked', 'in_transit', 'ofd', 'out_for_delivery'].includes(s)) return 'In Courier';
  if (['delivered', 'complete'].includes(s)) return 'Delivered';
  if (s.includes('return')) return 'Return Pending';
  return null;
}

export function buildTrackingUrl(courier: string | null, trackingCode: string | null, consignmentId: string | null): string | null {
  if (!trackingCode && !consignmentId) return null;
  const service = (courier || '').toLowerCase();
  switch (service) {
    case 'steadfast': return `https://steadfast.com.bd/t/${encodeURIComponent(consignmentId || trackingCode || '')}`;
    case 'pathao': return `https://merchant.pathao.com/tracking?consignment_id=${encodeURIComponent(consignmentId || trackingCode || '')}`;
    case 'redx': return `https://redx.com.bd/track-global-parcel/?trackingId=${encodeURIComponent(trackingCode || consignmentId || '')}`;
    case 'carrybee': return `https://merchant.carrybee.com/order-track/${encodeURIComponent(consignmentId || trackingCode || '')}`;
    default: return null;
  }
}
