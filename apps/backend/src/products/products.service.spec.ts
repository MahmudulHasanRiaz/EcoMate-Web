...
describe('ProductsService', () => {
    ...
    attributeValues: [
      {
        ...
        attributeValue: { id: 'attr-val-1', value: 'Red', attribute: { id: 'attr-1', name: 'Color' } },
      },
    ],
  }
  ...
  describe('findAll', () => {
    ...
    it('should filter by categoryId, type, and isActive', async () => {
      ...
      await service.findAll({ categoryId: 'cat-1', type: 'simple', isActive: true })
  ...
  describe('create', () => {
    ...
    it('should create a simple product without variants', async () => {
      ...
      ;(prisma.media.findFirst as jest.Mock).mockResolvedValue({ id: 'media-1', filename: 'img-new.jpg' })
    ...
    it('should create a product with variants', async () => {
      ...
      ;(prisma.product.findUnique as jest.Mock).mockResolvedValue(null)
      ;(prisma.product.create as jest.Mock).mockResolvedValue(productWithVariant)
    ...
    it('should attach media when images are provided and media found', async () => {
      ...
      expect(prisma.media.findFirst).toHaveBeenCalledWith({ where: { filename: 'img-new.jpg' } })
  ...
  describe('update', () => {
    ...
    it('should update a product successfully', async () => {
      ;(prisma.product.findUnique as jest.Mock).mockResolvedValueOnce(mockProduct).mockResolvedValueOnce(null)
      ;(prisma.product.update as jest.Mock).mockResolvedValue({ ...mockProduct, name: 'Updated Product', salePrice: 900 })
      ;(prisma.media.findFirst as jest.Mock).mockResolvedValue(null)
    ...
    it('should throw ConflictException if new slug already exists on another product', async () => {
      ...
      await expect(service.update('prod-1', { slug: 'taken-slug' })).rejects.toThrow(ConflictException)
    })
    ...
    it('should attach new media when images are provided', async () => {
      ...
      ;(prisma.product.update as jest.Mock).mockResolvedValue(mockProduct)
      ;(prisma.media.findFirst as jest.Mock).mockResolvedValue({ id: 'media-2', filename: 'new-image.jpg' })
      ;(media.attach as jest.Mock).mockResolvedValue(undefined)

      await service.update('prod-1', { images: ['https://cdn.example.com/new-image.jpg'] })
  ...
  describe('remove', () => {
    ...
    it('should throw Prisma error (NotFound) if product does not exist', async () => {
      ...
      ;(prisma.product.findUniqueOrThrow as jest.Mock).mockRejectedValue(notFoundError)

      await expect(service.remove('nonexistent-id')).rejects.toThrow('Not found')
  ...
  describe('generateVariants', () => {
    ...
    it('should generate variants from attribute combinations', async () => {
      ...
      ;(prisma.product.findUniqueOrThrow as jest.Mock).mockResolvedValue(product)
      ;(prisma.productVariant.deleteMany as jest.Mock).mockResolvedValue({ count: 0 })
      ;(prisma.attributeValue.findMany as jest.Mock).mockResolvedValue([
        { id: 'av-1', value: 'Red', attributeId: 'attr-1', attribute: { id: 'attr-1', name: 'Color' } },
        { id: 'av-2', value: 'Blue', attributeId: 'attr-1', attribute: { id: 'attr-1', name: 'Color' } },
        { id: 'av-3', value: 'S', attributeId: 'attr-2', attribute: { id: 'attr-2', name: 'Size' } },
        { id: 'av-4', value: 'M', attributeId: 'attr-2', attribute: { id: 'attr-2', name: 'Size' } },
      ])
      ;(prisma.productVariant.create as jest.Mock).mockResolvedValue(mockVariant)
      ;(prisma.product.update as jest.Mock).mockResolvedValue({ ...product, type: 'variable' })
      ;(prisma.product.findUnique as jest.Mock).mockResolvedValue({ ...product, type: 'variable', variants: [mockVariant] })
