...
const ERROR_MESSAGES: Record<string, string> = {
  ...
  unreachable: 'Cannot reach KeyMate server. Check KEYMATE_API_URL, server status, and network.',
  engine_unavailable: 'License engine failed to load. Please restart the application.',
  not_verified: 'License not yet verified. Please wait for initialization to complete.',
  ...
  keymate_unreachable: 'Cannot reach KeyMate server. Check KEYMATE_API_URL, server status, and network.',
  offline_cache: 'License server unreachable — using cached license. Some features may be limited.',
};
...
export class LicenseService implements OnModuleInit {
  ...
  async onModuleInit() {
    try {
      ...
      try {
        ...
        console.warn('[License] Failed to decrypt stored credentials — encryption key may have changed');
      ...
      if (err?.message?.toLowerCase().includes('fetch') || err?.message?.toLowerCase().includes('econnrefused') || err?.message?.toLowerCase().includes('network')) {
        console.warn('[License] KeyMate server unreachable — will retry on next request');
      } else if (err?.message?.toLowerCase().includes('decrypt')) {
        console.warn('[License] Failed to decrypt stored credentials');
      } else {
        console.warn('[License] Failed to validate license:', err?.message || err);
      }
    }
  }
  ...
  async activateWithKeymate(licenseKey: string, domain: string, apiKey?: string) {
    ...
    try {
      ...
      return {
        success: false,
        error: code,
        message: friendlyError(code, 'Activation failed. Please check your license key.'),
        license: lic,
      };
    } catch (err: any) {
      return {
  ...
  getStatus() {
    ...
    else if (lic.code === 'unreachable' || lic.code === 'offline_cache') state = 'offline';
