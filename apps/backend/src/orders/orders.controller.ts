import { Controller, Get, Post, Put, Delete, Body, Param, Query, Sse } from '@nestjs/common';
...
export class OrdersController {
  ...
  @Put(':id') updateOrder(@Param('id') id: string, @Body() dto: UpdateOrderDto) { return this.svc.updateOrder(id, dto); }
  ...
  @Post(':id/items') addItem(@Param('id') id: string, @Body() dto: UpdateOrderItemDto) { return this.svc.addItem(id, dto); }
  @Delete(':id/items/:itemId') removeItem(@Param('id') id: string, @Param('itemId') itemId: string) { return this.svc.removeItem(id, itemId); }
  @Post(':id/note') addNote(@Param('id') id: string, @Body() dto: { note: string; visibility: 'public' | 'private' }, @CurrentUser() user: { userId: string }) { return this.svc.addNote(id, dto.note, dto.visibility, user.userId); }
  ...
  streamUpdates(): Observable<MessageEvent> {
    ...
    const interval = setInterval(() => {
      subject.next({ data: JSON.stringify({ type: 'heartbeat', timestamp: new Date().toISOString() }) } as MessageEvent);
    }, 10000);
