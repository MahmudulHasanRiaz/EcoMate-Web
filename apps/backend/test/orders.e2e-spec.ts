...
describe('Orders (e2e)', () => {
  ...
  afterEach(async () => {
    try {
      ...
      await prisma.userSettings.deleteMany({ where: { user: { email: { startsWith: 'e2e-orders-' } } } })
  ...
  it('GET /orders/:id → 200 with order details and items', async () => {
    const order = await createOrder({ items: [{ productId, quantity: 3, price: 500 }] })
    ...
    expect(res.body.customer).toMatchObject({ firstName: customerUser.firstName })
  ...
  it('PUT /orders/:id/status → 200 when transitioning to valid next status', async () => {
    ...
    const currentStatus = statusesRes.body.find((s: any) => s.id === order.statusId)
