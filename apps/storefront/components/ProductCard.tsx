...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <div className="bg-white rounded-[8px] overflow-hidden flex flex-col h-full border border-gray-200 relative group transition-all">
      ...
      <div 
        className="relative w-full aspect-square bg-white flex items-center justify-center p-0 cursor-pointer overflow-hidden"
      ...
      >
        <motion.img
          ...
          src={imageError ? PLACEHOLDER_IMAGE : (product.image || PLACEHOLDER_IMAGE)}
          alt={product.name}
      ...
      <div className="p-2 md:p-3 flex flex-col flex-1 bg-white">
        <h4 
          className="text-[12px] md:text-[14px] text-gray-800 line-clamp-2 mb-2 font-medium leading-normal cursor-pointer hover:text-brand-blue transition-colors min-h-[2.2rem] md:min-h-[2.5rem]"
          onClick={() => router.push(`/products/${product.id}`)}
        >
          {product.name}
        </h4>
        ...
        <div className="mt-auto space-y-3">
          <div className="flex items-center gap-1.5 flex-wrap">
            <span className="text-[15px] md:text-[18px] font-black text-brand-blue">
              ৳{product.price.toLocaleString()}
            </span>
            {product.originalPrice && (
              <span className="text-[12px] md:text-[14px] font-medium text-gray-300 line-through">
          ...
          {inCart ? (
            <div className="flex items-center h-[34px] md:h-[40px] w-full border-2 border-brand-blue/20 rounded-lg overflow-hidden">
              <button
                ...
                className="w-10 h-full bg-white text-brand-blue border-r border-brand-blue/10 flex items-center justify-center font-black hover:bg-brand-blue/5 transition-colors"
              >
                -
              </button>
              <div className="flex-1 h-full flex items-center justify-center font-black text-brand-blue bg-brand-blue/5 text-[14px]">
                {quantity}
              </div>
              <button
                onClick={() => updateQuantity(product.id, quantity + 1)}
                className="w-10 h-full bg-white text-brand-blue border-l border-brand-blue/10 flex items-center justify-center font-black hover:bg-brand-blue/5 transition-colors"
              >
                +
              </button>
            </div>
