...
export function StoreAppCard({
  ...
  useEffect(() => {
    ...
    if (localStorage.getItem('pwa-installed')) setInstalled(true);
  ...
  const renderAndroidBtn = () => {
    // Priority 1: Play Store URL configured → link
    if (playStoreUrl) {
      return (
    ...
    // Priority 3: On Android with install prompt available → trigger install
    ...
    // Priority 4: Not on Android — show disabled indicator
    return (
  ...
  const renderIosBtn = () => {
    // Priority 1: App Store URL configured → link
    if (appStoreUrl) {
      return (
    ...
    // Priority 3: On iOS — show App Store link or instructions
    if (platform === 'ios') {
      return (
        <span className="inline-flex items-center gap-1.5 bg-gray-100 text-gray-500 text-xs font-bold px-4 py-2.5 rounded-xl">
          <svg width="16" height="16" viewBox="0 0 512 512" fill="currentColor">
    ...
    // Priority 4: Not on iOS — show disabled indicator
    return (
      <span className="inline-flex items-center gap-1.5 bg-gray-100 text-gray-500 text-xs font-bold px-4 py-2.5 rounded-xl">
        <svg width="16" height="16" viewBox="0 0 512 512" fill="currentColor">
          <path d="M318.7 268.7c-.2-36.7 16.4-64.4 50-84.8-18.8-26.9-47.2-41.7-84.7-44.6-35.5-2.8-74.3 20.7-88.5 20.7-15 0-49.4-19.7-76.4-19.7C63.3 141.2 4 184.8 4 273.5q0 39.3 14.4 81.2c12.8 36.7 59 126.7 107.2 125.2 25.2-.6 43-17.9 75.8-17.9 31.8 0 48.3 17.9 76.4 17.9 48.6-.7 90.4-82.5 102.6-119.3-65.2-30.7-61.7-111.3-59.9-121.2z"/>
