...
- Requires cross-surface configuration consistency: any shared brand/identity setting (e.g. a storefront header logo-vs-name mode configured in settings) must be the single source of truth for every surface that renders that brand — including templates/stickers — so they stay in sync rather than being hard-coded independently per surface. Sticker templates must mirror the exact header setting (logo-only → logo-only; logo+name → logo+name; name-only → name-only). Confidence: 0.95
- Requires precise centering of compound brand elements when a logo/icon and business name render together (e.g. sticker/order-print templates): the icon must use context-appropriate sizing (a small favicon, not a wide banner-logo class) so the pair sits centered together; the layout must never let a wide element push text to the far edge, and centering of the compound element must be exact, not approximate. Confidence: 0.82
- Requires reconciliation/comparison systems to use a configuration-aware waterfall model — not flat counts but multi-stage pipeline showing how numbers flow through classification stages (Business → Source Classification → Eligible/Excluded → Expected → Canonical → Matched/Missing/Unexpected). Confidence: 0.95
- Every numerical discrepancy must be classified into exactly one of: (1) Configuration-intentional, (2) Expected/valid system behavior, (3) Actual anomaly requiring investigation. No unclassified differences allowed in reports. Confidence: 0.95
- Aggregate anomaly counts require drill-down to specific entity IDs (order IDs, snapshot IDs, dispatch IDs). Numbers alone are insufficient — operators must be able to trace anomalies to individual records. Confidence: 0.9
- Different monitoring metric scopes must never be mixed on the same page without explicit labels: Business/Order scope, Tracking Pipeline scope, Runtime Health scope, Historical Backlog scope — each gets its own clearly-labeled section. Confidence: 0.9
- When customer data is edited from the order detail page, the edit must update BOTH the mutable CustomerProfile AND the immutable Order snapshot fields — because the edit is a correction of the order record, not a generic CustomerProfile mutation. Confidence: 0.9
- Customer name on order detail pages should be a clickable link to the customer profile page (when customerId exists), enabling quick navigation for operators. Confidence: 0.85
